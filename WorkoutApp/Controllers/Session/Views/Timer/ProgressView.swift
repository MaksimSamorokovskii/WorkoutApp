...
//  WorkoutApp
//
//  Created by Максим Самороковский on 05.07.2023.
//
...
extension TimerView {
   final class ProgressView: UIView {
        func drawProgress(with percent: CGFloat) {
            ...
            let dotAngele = CGFloat.pi * (7 / 6 - (8 / 6 * percent))
            let dotPoint = CGPoint(x: cos(-dotAngele) * radius + centr.x,
                                   y: sin(-dotAngele) * radius + centr.y)
            ...
            let barsPath = UIBezierPath(arcCenter: centr,
                                        radius: barsRadius,
                                        startAngle: startAngle,
                                        endAngle: endAngle,
                                        clockwise: true)
            ...
            (1...9).forEach { _ in
                ...
                let startBarPoint = CGPoint(
                    x: cos(-barAngle) * startBarRadius + centr.x,
                    y: sin(-barAngle) * startBarRadius + centr.y
                )
            
            let endBarPoint = CGPoint(
                x: cos(-barAngle) * endBarRadius + centr.x,
                y: sin(-barAngle) * endBarRadius + centr.y
            )
